...
//The ending coordinates in the maze: er, ec; the row number and column number range from 0 to 9
{
    ...
    coordStack.push(Coord(sr,sc)); //pushed starting coord into queue

    
    while ( !coordStack.empty() ) //while the queue is not empty
    {
        Coord currentLocation = coordStack.front();
        const int currentRow = currentLocation.r();
        const int currentColumn = currentLocation.c();
 
        coordStack.pop(); //remove the front of the queue
